...
class GeminiLiveService(
    ...
    fun finishUserTurn(wavBase64: String? = null) {
        if (_liveState.value == LiveState.LISTENING || _liveState.value == LiveState.PROCESSING) {
            ...
            val effectiveWav = wavBase64 ?: audioRecorder.getAndResetTurnSpeechWavBase64()
            val key = lastApiKey
            if (!effectiveWav.isNullOrBlank() && key.isNotBlank() && key != "MY_GEMINI_API_KEY") {
                processUserAudioTurn(effectiveWav, key)
            }
        }
    }

    private fun processUserAudioTurn(wavBase64: String, apiKey: String) {
        activeSpeechTurnJob?.cancel()
