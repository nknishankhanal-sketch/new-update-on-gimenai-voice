...
class AudioRecorder {
    ...
    companion object {
        ...
        // VAD Constants (Responsive speech detection) - Optimized for low latency
        private const val VAD_SPEECH_ONSET_THRESHOLD = 0.008f
        private const val VAD_SPEECH_CONTINUE_THRESHOLD = 0.004f
        private const val VAD_BARGE_IN_THRESHOLD = 0.08f
        private const val MIN_SPEECH_DURATION_MS = 100L
        private const val SILENCE_TIMEOUT_MS = 300L
    ...
    fun getAndResetTurnSpeechWavBytes(): ByteArray? {
        ...
        return createWav(pcmBytes, SAMPLE_RATE)
    }

    @SuppressLint("MissingPermission")
    fun startRecording(
        onAudioChunk: (ByteArray) -> Unit,
        onSilenceAfterSpeech: ((ByteArray?) -> Unit)? = null,
    ...
    private fun processVadFrame(
        ...
        frameVolume: Float,
        onAudioChunk: (ByteArray) -> Unit,
        onSilenceAfterSpeech: ((ByteArray?) -> Unit)?
    ) {
        ...
        if (isVoiceActive) {
            ...
            // Always forward the current audio frame (binary)
            onAudioChunk(frameBytes)
            ...
            if (isConfirmedSpeech) {
                ...
                if (silenceStartTimeMs == 0L) {
                    ...
                    val wavBytes = getAndResetTurnSpeechWavBytes()
                    resetVadState()
                    onSilenceAfterSpeech?.invoke(wavBytes)
                }
            } else {
