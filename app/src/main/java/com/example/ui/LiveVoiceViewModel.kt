...
open class LiveVoiceViewModel : ViewModel() {
    ...
    private val connectionManager: ConnectionManager by lazy {
        ConnectionManager(
            liveClient = liveService.liveClientInstance,
            liveService = liveService,
            ttsSynthesizer = geminiVoiceSynthesizer,
            audioPlayer = audioPlayer,
            scope = viewModelScope
        )
    }
    val connectionState: StateFlow<ConnectionManager.ConnectionState> = connectionManager.state
    ...
    fun toggleLiveSession() {
        ...
        if (currentState is ConnectionManager.ConnectionState.Disconnected || 
            currentState is ConnectionManager.ConnectionState.Error) {
            connectLive()
        } else {
            disconnectLive()
        }
    ...
    fun connectLive() {
        ...
        connectionManager.connect(apiKey, _selectedVoice.value, _selectedPersona.value)
    }
    ...
    fun sendTextMessage(text: String) {
        ...
        liveService.addMessage(userMsg)  // We'll need to add this function
        
        viewModelScope.launch {
            val result = connectionManager.sendUserMessage(text)
            result.onFailure { error ->
                // Error already shown via connectionState banner
                liveService.addSystemMessage("❌ ${error.userMessage}")
            }
        }
    }
        ...
        if (_isMuted.value) {
            ...
            audioRecorder.startRecording(
                onAudioChunk = { pcmBytes ->
                    liveService.sendRealtimeAudioFrame(pcmBytes)
                },
                onSilenceAfterSpeech = { wavBytes ->
                    liveService.finishUserTurn(wavBytes)
                }
            )
        }
    }
    ...
    fun testVoiceAudio() {
        ...
        if (apiKey.isNotBlank()) {
            viewModelScope.launch {
                connectionManager.speakText(
                    "नमस्ते साथी! म नेक्सस एआई हुँ। तपाईंको आवाज प्रणाली एकदमै राम्रोसँग काम गरिरहेको छ नि!", 
